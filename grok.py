...
# 代理池：每个注册轮次换一个出口 IP，绕开 xAI 每-IP 验证码限额
PROXY_POOL = []
_proxy_index = 0
...
def load_proxy_pool():
    global PROXY_POOL
    path = os.getenv("GROK_PROXY_LIST", "")
    if not path and os.path.exists("keys/webshare.txt"):
        path = "keys/webshare.txt"
    if not path and os.path.exists("keys/good_proxies.txt"):
        path = "keys/good_proxies.txt"
    if not path or not os.path.exists(path):
        return
    with open(path) as f:
        PROXY_POOL = [l.strip() for l in f if l.strip().startswith("http")]
...
def get_next_proxy():
    ...
    with _proxy_lock:
        p = PROXY_POOL[_proxy_index % len(PROXY_POOL)]
        _proxy_index += 1
    return p
...
def register_single_thread(email_provider: str = "gptmail"):
    ...
    while not stop_event.is_set():
        try:
            # 每个注册轮次换一个出口 IP，绕开每-IP 验证码限额
            this_proxy = get_next_proxy()
            this_proxies = {"http": this_proxy, "https": this_proxy} if this_proxy else None
            with requests.Session(impersonate="chrome120", proxies=this_proxies) as session:
                # 预热连接
...
def main():
    ...
    # 加载代理池，绕开每-IP 验证码限额
    load_proxy_pool()
    print(f"[*] 代理池: {len(PROXY_POOL)} 个代理 (GROK_PROXY_LIST)" if PROXY_POOL else "[*] 代理池: 空，直连")
    ...
    with requests.Session(impersonate="chrome120", proxies=PROXIES) as s:
        try:
            ...
            def _fetch_and_search(url):
                ...
                try:
                    js = _req.get(url, proxies={"http": _proxy_url, "https": _proxy_url} if _proxy_url else None, timeout=10).text
