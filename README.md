...
## Fork: Personal Unlimited Grok (Aidil's setup)

Patches on top of upstream that made this work end-to-end on a VPS (no Clash, no paid email):

### Key fixes
- **No-proxy default**: `_proxy_url = os.getenv("GROK_PROXY", "") or ""` and `PROXIES` is `None` when empty. Upstream hardcodes `http://127.0.0.1:7897` (Clash) which breaks on a plain VPS.
- **Proxy pool rotation** (`grok.py`): loads `keys/good_proxies.txt` (or `GROK_PROXY_LIST` env) and rotates to a fresh proxy per registration attempt. xAI throttles validation-code emails **per IP**, so one proxy per account bypasses the limit.
- **mail.tm inbox provider** (`email_service.py`): free API-based temp inbox, no browser needed. Critical because xAI throttles per **Gmail base account** (all `+alias` addresses share the mailbox) after ~5-6 code sends.
- **Gmail fetch fix** (`email_service.py`): `fetch_first_email` now returns `Subject:` first (the clean `594-475` format) instead of raw HTML body. The HTML body contains CSS noise like `333333` that pollutes the code regex.
- **Recipient-precise fetch**: search by `TO "<alias>"` only. Never fall back to "latest x.ai email" — that grabs another alias's code and produces `Email validation code is invalid`.

### Personal gateway (`grok_proxy.py`)
OpenAI-compatible proxy that serves all farmed accounts behind one endpoint:
- Loads `auths/xai-*.json`, round-robins across accounts
- Auto-refreshes tokens 5 min before expiry
- Exposes `/v1/models`, `/v1/chat/completions` (streaming), `/v1/images/generations`
- Auth: `Authorization: Bearer <GROK_PROXY_KEY>`

```bash
python -m uvicorn grok_proxy:app --host 127.0.0.1 --port 8099
```

### Minting all accounts (`mint_all.py`)
Reads `keys/accounts.txt` (`email:password:sso`), does the PKCE consent flow per account (SSO cookie → auth code → access/refresh tokens), writes `auths/xai-<email>.json`.

### Real costs
- YesCaptcha: **25 points per account** (¥0.025 / RM0.015) — one Turnstile solve per registration
- Email: mail.tm free
- 6 accounts ≈ ¥0.15 total captcha

### Usage
```bash
# 1. Configure
cp .env.example .env   # set YESCAPTCHA_KEY, GMAIL creds or use mailtm
echo 'http://user:pass@ip:port' > keys/good_proxies.txt   # proxy pool, one per account

# 2. Register N accounts
python grok.py --email-provider mailtm --count 3 --threads 1
...
### Account lifespan
xAI bans free accounts in waves (especially around model releases) and revokes refresh tokens. Re-register when tokens 401. Costs ~30s per account.
